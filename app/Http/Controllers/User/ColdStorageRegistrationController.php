  ...
  public function Taluka_List(Request $request)
       ...
       public function check_application_status(Request $request)
    {
        ...
        { 
            ...
            return view('user.home', compact('get_details','type'));

        }

        if(!empty($get_renewal_application_status))
        { 
    ...
    public function createss()
    {
        ...
        //  dd($data);
         if(!empty($data)) {
              ...
              return view('user.cold_storage.cold_storage_registration', compact('meattype_mst','unit_Meat_Type'));
       ...
       public function updateColdStorage(Request $request, $id)
    {
        ...
        $this->validate($request, [
            ...
            'issued_doc' => 'mimes:jpeg,png,jpg,pdf|max:2048',
        ...
        if(!empty($request->hasFile('Factory_reg_and_license_doc'))){
            $image = $request->file('Factory_reg_and_license_doc');
            $image_name = $image->getClientOriginalName();
            $extension = $image->getClientOriginalExtension();
            $new_name = time().rand(10,999).'.'.$extension;
            $image->move(public_path('/PMC_Cold_Storage/meat_file/Factory_reg_and_license_doc'),$new_name);

            $image_path = "/PMC_Cold_Storage/meat_file/Factory_reg_and_license_doc" . $image_name;
            $data->Factory_reg_and_license_doc = $new_name;
        
         }else{
            unset($data->Factory_reg_and_license_doc);
        }
    ...
    //     if ($data->hod_status == 2) {
    //         $data->hod_status = 0;
    //     }
        
    //     if($data->final_approve == 2){
    //         $data->final_approve = 0;
    //     }
    ...
    //   }else{
        ...
        'areaof_business_place' => 'required|string',
        'business_place' => 'required|numeric',
        // Uncommented fields (conditionally required based on 'business_place')
    ];

    // Conditional rules based on 'business_place' value
    if ($request->input('business_place') == 2) {
        $rules = array_merge($rules, [
            'adharcard_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            'residitional_proof_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            // 'municipal_corpor_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            'legal_business_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            'business_registration_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            // 'Factory_reg_and_license_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            'property_tax_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            'paid_water_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            'treatment_authorized_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            'fitness_certificate_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            'issued_doc' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
            'slaughter_letter_doc' => 'mimes:jpeg,png,jpg,pdf|max:10240',
            'applicant_signature' => 'required|mimes:jpeg,png,jpg|max:10240',
            'profile_photo' => 'required|mimes:jpeg,png,jpg,pdf|max:10240',
        ]);
    }
    ...
    $this->validate($request, $rules, [
        ...
        'business_address.required' => 'Address of the business is required',
        'sewerage_disposing.required' => 'Provision of sewerage for disposing effluent is required',
        'place_id.required' => 'Is place is located at least 50mt. away from place of worship/educational institute/hospital & clinic is required',
        'regi_authority_name.required' => 'Registration authority name is required',
        'register_number.required' => 'Registration number is required',
        'valid_till.required' => 'Valid till Date is required',
        'areaof_business_place.required' => 'Area of business place is required',
        'business_place.required' => 'Business place is required',
        // Upload Document
        'adharcard_doc.required' => 'Applicant Adharcard is required',
        'adharcard_doc.max' => 'The file size should be less than 10MB.',
        'adharcard_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'residitional_proof_doc.required' => 'Residential proof document is required',
        'residitional_proof_doc.max' => 'The file size should be less than 10MB.',
        'residitional_proof_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'legal_business_doc.required' => 'Legal document of the business place is required',
        'legal_business_doc.max' => 'The file size should be less than 10MB.',
        'legal_business_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'municipal_corpor_doc.required' => 'Municipal Corporation Permission Letter is required',
        'municipal_corpor_doc.max' => 'The file size should be less than 10MB.',
        'municipal_corpor_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'business_registration_doc.required' => 'Business registration certificate is required',
        'business_registration_doc.max' => 'The file size should be less than 10MB.',
        'business_registration_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'Factory_reg_and_license_doc.required' => 'Factory registration and license to operate the factory is required',
        'Factory_reg_and_license_doc.max' => 'The file size should be less than 10MB.',
        'Factory_reg_and_license_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'property_tax_doc.required' => 'Receipt of recently paid property tax is required',
        'property_tax_doc.max' => 'The file size should be less than 10MB.',
        'property_tax_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'paid_water_doc.required' => 'Receipt of recently paid water is required',
        'paid_water_doc.max' => 'The file size should be less than 10MB.',
        'paid_water_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'treatment_authorized_doc.required' => 'Pest control treatment certificate issued from authorized agency is required',
        'treatment_authorized_doc.max' => 'The file size should be less than 10MB.',
        'treatment_authorized_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'fitness_certificate_doc.required' => 'Medical fitness certificate issued by Municipal hospital is required',
        'fitness_certificate_doc.max' => 'The file size should be less than 10MB.',
        'fitness_certificate_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'issued_doc.required' => 'Document issued by APEDA, MPCB(ETP), FSSAI is required',
        'issued_doc.max' => 'The file size should be less than 10MB.',
        'issued_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        // 'slaughter_letter_doc.required' => 'Details & authority letter from authorized slaughter house/poultry form & authority letter is required',
        'slaughter_letter_doc.max' => 'The file size should be less than 10MB.',
        'slaughter_letter_doc.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
        'applicant_signature.required' => 'Applicant signature is required',
        'applicant_signature.max' => 'The file size should be less than 10MB.',
        'applicant_signature.mimes' => 'Only files in .jpg, .jpeg, .png format can be uploaded.',
        'profile_photo.required' => 'Applicant profile photo is required',
        'profile_photo.max' => 'The file size should be less than 10MB.',
        'profile_photo.mimes' => 'Only files in .jpg, .jpeg, .png, .pdf format can be uploaded.',
    ]);
        ...
        if(!empty($request->hasFile('municipal_corpor_doc'))){
            $image = $request->file('municipal_corpor_doc');
            $image_name = $image->getClientOriginalName();
            $extension = $image->getClientOriginalExtension();
            $new_name = time().rand(10,999).'.'.$extension;
            $image->move(public_path('/PMC_Cold_Storage/meat_file/municipal_corpor_doc'),$new_name);

            $image_path = "/PMC_Cold_Storage/meat_file/municipal_corpor_doc" . $image_name;
            $data->municipal_corpor_doc = $new_name;
        }
    ...
    public function User_ApplicationForm_View(Request $request, $application_id, $user_type)
    {
        ...
        if($user_type == 'Cold_Storage')
        {
            ...
            // return $data;
            // dd($data); 
            return view('user.cold_storage.user_applicant_form_view', compact('data', 'user_type','meattype_mst','unit_Meat_Type'));
        }
    ...
    public function coldRegistrationInvoice(Request $request, $application_id, $user_type)
    {
         $invoice =  DB::table('coldstorage_registration_tbl AS t1')
                        ...
                        ->orderBy('t1.id', 'DESC')
                        ->first();
        return view('user.cold_storage.invoice',compact('invoice'));
    }
       ...
       public function ApplicationForm_View(Request $request, $application_id, $user_type)
    {
        ...
        return view('user.cold_storage.applicant_coldstorage_view', compact('meat_registration_view','unit_Meat_Type', 'commaSeparatedMeatNames'));
    }
       ...
       public function GenerateenglishLicensepdf(request $request, $id)
    {
        ...
        return view('user.cold_storage.generate_english_coldstorage_license_pdf', compact('meat_registration_pdf','fiscalYear','unit_Meat_Type'));
    }
     ...
     public function GenerateMarathiLicensepdf(request $request,$id)
      {
        ...
        return view('user.cold_storage.generate_marathi_coldstorage_license_pdf', compact('meat_registration_pdf','fiscalYear','unit_Meat_Type'));
       }
